...
function parseRows(rows: string[][]): Student[] {
  ...
  if (looksLikeHeader) {
    ...
    // Only treat a column as the ID column when the header really is an id
    // ("id", "student id", "id #") — never a word that merely contains "id".
  ...
  return body
    .map((row) => {
      ...
      // Stable IDs so printed QR codes keep working when rows move around.
...
export const getRoster = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<RosterResult> => {
    ...
    // Encode spaces only — ! and : must stay literal in the A1 range.
